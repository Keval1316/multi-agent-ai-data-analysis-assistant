...
class InsightItem(BaseModel):
    ...
    what_this_means: Optional[str] = Field(None, description="Plain-English explanation of what the numbers mean for non-technical readers")
    interpretation: str = Field(..., description="Business or operational implication of the observed finding")
    implication: str = Field(..., description="Recommended action or investigation step (using prudent verbs like investigate, review, monitor)")
    confidence: str = Field("High", description="'High', 'Moderate', 'Low', or 'Caveat'")
    confidence_rationale: Optional[str] = Field(None, description="Why this confidence level was assigned based on sample size and variable completeness")
    statistical_vs_practical: Optional[str] = Field(None, description="Distinction between statistical significance and practical effect size")
    ...
    category: str = Field("Inventory & Operations", description="e.g. 'Inventory & Operations', 'Distribution Profile', 'Relationships & Trends', 'Data Quality'")
